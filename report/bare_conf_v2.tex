...
A complexidade do algoritmo depende do número de comparações e trocas realizadas. Em todos os casos, o número máximo de comparações realizadas é:
\[
(n - 1) + (n - 2) + \ldots + 1 = \frac{n(n - 1)}{2}
\]
o que corresponde a uma complexidade de ordem \(\mathcal{O}(n^2)\).

No melhor caso, quando a lista já está ordenada, a flag \texttt{swapped} permite terminar após a primeira iteração sem trocas. Neste cenário, o algoritmo apenas percorre uma vez a lista e não realiza nenhuma troca, resultando numa complexidade de tempo linear:
\[
\mathcal{O}(n)
\]

No pior caso, quando a lista está em ordem inversa, o algoritmo realiza o número máximo possível de trocas e comparações, resultando em:
\[
\mathcal{O}(n^2)
\]

Apesar da sua simplicidade, o Bubble Sort é ineficiente para listas grandes. No entanto, é estável e pode ser útil apenas para fins didáticos ou em contextos onde a simplicidade do código é prioritária.
